...
class DecisionConsistencyMetric(BaseMetric):
    ...
    def __init__(self, threshold: float = 0.67, n_runs: int = N_RUNS):
        ...
        self.score = 0.0
        self.reason = "Metric not evaluated."
        self.success = False
...
def _build_cross_run_coherence_metric() -> GEval:
    """Create a fresh GEval metric instance per scenario."""
    return GEval(
...
def _fmt_status(metric: BaseMetric) -> str:
    try:
    ...
    for scenario_name, question, prompt_key in CONSISTENCY_SCENARIOS:
        ...
        try:
            ...
            markdown_table = (
                ...
                "|--------|-------|--------|--------|\n"
                f"| {decision_metric.name} | {_fmt_score(getattr(decision_metric, 'score', None))} | "
                f"{_fmt_status(decision_metric)} | "
                f"{_fmt_reason(getattr(decision_metric, 'reason', None))} |\n"
                f"| {coherence_metric.name} | {_fmt_score(getattr(coherence_metric, 'score', None))} | "
                f"{_fmt_status(coherence_metric)} | "
                f"{_fmt_reason(getattr(coherence_metric, 'reason', None))} |"
            )

            allure.attach(
                question,
                name=f"{scenario_name} - 1. User Input",
