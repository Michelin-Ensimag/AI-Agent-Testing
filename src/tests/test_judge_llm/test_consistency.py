...
def test_consistency():
    ...
    dataset = EvaluationDataset(goldens = [Golden(input=question), 
                                           Golden(input="Si j'entre sur AAPL à $180,"
                                           ...
                                           Golden(input="Compte tenu d'un portefeuille conservateur," \
                                           " quel est le risque de concentration si j'achète du AAPL maintenant ?"),
                                           Golden(input="L'avance de la concurrence sur l'IA menace-t-elle la position dominante d'Apple à long terme ?")])
