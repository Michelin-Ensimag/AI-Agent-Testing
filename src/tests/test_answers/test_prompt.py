"""
test_agent_pred_prompt.py — Prompt comparison with a single DeepEval table
    ...
    for golden in dataset.goldens:
        ...
        test_cases.append(
            LLMTestCase(
                ...
                metadata={"system_prompt": system_prompt[:60]},
            )
        )
...
def test_prompt_evaluation():
    ...
    dataset = EvaluationDataset(
        goldens=[
            Golden(
                name="EN Minimal",
                input="Analyze Apple (AAPL) and give me a trading strategy.",
                additional_metadata={"sys": STRATEGY_SYSTEM_EN},
            ),
            Golden(
                name="FR Minimal",
                input="Analyse Apple (AAPL) et donne-moi une stratégie de trading.",
            ...
            Golden(
                name="EN Typos",
                input="anlyze aple (AAPL) stok and giv me a trding strategey plz",
                additional_metadata={"sys": STRATEGY_SYSTEM_EN},
            ),
            Golden(
                name="FR Typos",
                input="analise aple (AAPL) et done moi une straategie de tradding stp",
                additional_metadata={"sys": STRATEGY_SYSTEM_FR},
            ),
            Golden(
                name="EN TSLA",
                input="Generate a trading strategy for Tesla (TSLA).",
                additional_metadata={"sys": STRATEGY_SYSTEM_EN},
            ),
            Golden(
                name="FR MSFT",
    ...
    evaluate(
        ...
        metrics=[task_completion_metric, strategy_metric],
    )
